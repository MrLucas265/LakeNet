...
public class CurContracts : MonoBehaviour 
{
	...
	void SendAccountDetailsEmail()
	{
		...
		GameControl.control.StoredLogins.Add (new LoginSystem ("LEC Bank", def.GetRandomNumber (6, 6), def.GetRandomString (15, 15), 0));
...
//		email.ContractSelect = -1;
    ...
    public void Complete()
    {
		...
		case MissionSystem.MissionType.PTCopy:
			if (AttachtedFile != "") 
			{
				if (AttachtedFile == GameControl.control.Contracts [Select].File)
				{
					...
					RemoveRankMission();
					GameControl.control.StoredLogins.Add (new LoginSystem ("REVA", def.GetRandomNumber (4, 4), def.GetRandomString (12, 12), 0));
		...
		case MissionSystem.MissionType.TCopy:
            ...
            case MissionSystem.MissionType.PTDelete:
			RemoteFileCheck();
			for (int i = 0; i < FileNames.Count; i++) 
			{
				if (FileNames.Contains(GameControl.control.Contracts [Select].File))
				...
				else 
				{
					...
					case 1:
						EmailSubject = "Contract Completion";
						EmailContent = "We can see you have sucessfully deleted the file your now an offical member please login at www.reva.com";
						SelectedResponse = 0;
						RemoveRankMission();
						break;
					case 2:
						EmailSubject = "Contract Completion";
						EmailContent = "We can see you have deleted the file we asked we have made you an account which means your now an offical member please login at www.reva.com";
						SelectedResponse = 0;
						RemoveRankMission();
						break;
					}
                    GameControl.control.Rep[0].RepLevel++;
					GameControl.control.LoginUser.Add("User: " + def.GetRandomNumber(6,6));
					GameControl.control.LoginPass.Add(def.GetRandomString(12,12));
					EmailSubject = "REVA Login Details";
					EmailContent = "Your new account has been created in this email you will find your login details for www.reva.com this is where you can find contracts,software,hardware upgrades once again congratz on being a new member" + 
					"\n" + "Username: " + GameControl.control.LoginUser[1] + 
					"\n" + "Password: " + GameControl.control.LoginPass[1];
					//SendAccountDetailsEmail();
//					EmailSender = "www.tugs.com";
//					EmailSubject = "Tugs Details";
//					EmailContent = "Hello we at tugs have detected that your running on dial up please go to www.tugs.com to upgrade your package to something a bit better then dial-up were looking foward to your services.";
//					SendEmail();
					FileNames.RemoveRange (0, FileNames.Count);
				}
			}
