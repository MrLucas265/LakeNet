	...
	public void SpaceCheckV2()
	{
		for (int i = 0; i < GameControl.control.ProgramFiles.Count; i++) 
		{
			for (int j = 0; j < CachedDrives.Count; j++)
			{
				if (GameControl.control.ProgramFiles [i].Type == ProgramSystem.ProgramType.Dir) 
				{
					if (GameControl.control.ProgramFiles [i].Name == CachedDrives [j]) 
					{
						Index = i;
					}
				}

				GameControl.control.ProgramFiles[Index].Free = GameControl.control.ProgramFiles[Index].Capacity - GameControl.control.ProgramFiles[Index].Used;

				if (GameControl.control.ProgramFiles [i].Type == ProgramSystem.ProgramType.Exe || GameControl.control.ProgramFiles [i].Type == ProgramSystem.ProgramType.Txt) 
				{
					if (GameControl.control.ProgramFiles[i].Location.StartsWith(CachedDrives[j]))
					{
						if (UsedSpace.Count > 0)
						{
							GameControl.control.ProgramFiles[Index].Used = 0;
							GameControl.control.ProgramFiles[Index].Used += GameControl.control.ProgramFiles[i].Used;
						}
					}
				}
			}
