...
public class LucOSDesktop : MonoBehaviour 
{
	...
	public float DesktopPos;

	public Texture2D ChannelLogo;
	public float ApplicationPadding;

	public List<float> ListOfBarPos = new List<float>();
	...
	public void UpdateUI()
	{
		...
		Group.height = 21 * Customize.cust.UIScale / 1.5f;
		...
		AppButton = new Rect(2 * Scale,DesktopY + 5,Group.width,Group.height+8);
		...
		TaskBarBG = new Rect(0 * Scale, DesktopY + 3, Screen.width, Group.height+12);
		...
		TaskBarHozScroll = new Rect(32, AppButton.y, Math, Group.height + 8);

		//5, 250, 100, 20

		DesktopPos = Group.height * 2.5f;
		updateUI = false;
	}
	...
	void TaskBarUI()
	{
		...
		TaskBarHozScroll.x = AppButton.x + BarWidth;
		ListOfBarPos.RemoveRange(0, ListOfBarPos.Count);

		for (int i = 0; i < TaskView.RunningTasks.Count; i++)
		{
			ListOfBarPos.Add(TaskBarHozScroll.x + TaskBarHozScroll.x * i + ApplicationPadding);
			if (GUI.Button(new Rect(ApplicationPadding + ListOfBarPos[i], TaskBarHozScroll.y, BarWidth, TaskBarHozScroll.height), TaskView.RunningTasks[i].ProgramName))
			{
