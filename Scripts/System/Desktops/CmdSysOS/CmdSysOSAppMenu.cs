...
public class CmdSysOSAppMenu : MonoBehaviour
{
	...
	void UpdateUI()
	{
		...
		SearchBar = new Rect(1 * Scale,SystemButtonsY,79 * Scale,20 * Scale);

		ShowAllButton = new Rect(1 * Scale,SystemButtonsY - 24,104 * Scale,20 * Scale);

		QuickList = new Rect (3 * Scale, 3 * Scale, 145 * Scale, 231);
		...
		if (show == true) 
		{
			...
			if (SearchSites == "")
			{
				if (ShowAllApps == false) 
				{
					...
					//{
					//	if(GUI.Button(new Rect(0 * Scale,scrollsize * 21,120 * Scale,ButtonHeight),GameControl.control.QuickProgramList[scrollsize].Name))
					//	{
					//		PlayClickSound();
				...
				if (ShowAllApps == true) 
				{
					...
					{
						if(GUI.Button(new Rect(0 * Scale,scrollsize * 21,120 * Scale,20),ListOfPrograms[scrollsize]))
						{
							PlayClickSound();
